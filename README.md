...
| 数据库 | MySQL |
| 认证 | JWT 双令牌（access 15min + refresh 7d rotation） + bcryptjs |
...
│  ┌───────┴─────────────────────────────┐          │
│  │      Prisma 7 + MySQL               │          │
│  │  User / RefreshToken / MoodRecord / │          │
│  │  Memory / ChatSession / ChatMessage /│          │
│  │  Article                            │          │
│  └─────────────────────────────────────┘          │
└───────────────────────────────────────────────────┘
                     │
...
| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/auth/login` | 登录（返回 access + refresh 双令牌） |
| POST | `/auth/register` | 注册 |
| POST | `/auth/refresh` | 刷新令牌（token rotation） |
| POST | `/auth/logout` | 退出登录（撤销 refresh token） |
| GET | `/auth/me` | 验证 Token |
| PUT | `/auth/profile` | 更新个人资料 |
| PUT | `/auth/password` | 修改密码（撤销所有 refresh token） |
...
| **RefreshToken** | 刷新令牌 | userId, token（SHA-256 hash）, expiresAt |
